...
export default async function OrdersPage({searchParams}:{searchParams:Promise<{page?:string}>}){
  ...
  return <><AdminNav/><h1>Bestellungen.</h1><div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Bestellung</th><th>Datum</th><th>Kunde</th><th>Total</th><th>Zahlung</th><th>Fulfillment</th><th>Tracking</th></tr></thead><tbody>{(data as (Order&{shipments:Pick<Shipment,'tracking_number'|'carrier'>[]})[]).map(o=><tr key={o.id}><td><Link href={`/admin/orders/${o.id}`}>{o.order_number}</Link></td><td>{formatDate(o.created_at)}</td><td>{o.email||'Checkout offen'}</td><td>{formatCHF(o.total_cents)}</td><td>{statusLabel(o.payment_status)}</td><td>{statusLabel(o.fulfillment_status)}</td><td>{o.shipments.map(s=>s.tracking_number).join(', ')||'–'}</td></tr>)}</tbody></table>{!data.length&&<p className="admin-card">Noch keine Bestellungen.</p>}</div><div className="admin-pagination">{page>1&&<Link href={`/admin/orders?page=${page-1}`}>← Zurück</Link>}{(count||0)>page*25&&<Link href={`/admin/orders?page=${page+1}`}>Weitere Bestellungen →</Link>}</div></>;
}
