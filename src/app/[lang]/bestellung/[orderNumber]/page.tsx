...
import { formatCHF,formatDate } from '@/lib/status';
import { isLocale } from '@/i18n/locales';
import { getDictionary } from '@/i18n/get-dictionary';
export async function generateMetadata({params}:{params:Promise<{lang:string}>}):Promise<Metadata>{
  const {lang}=await params;if(!isLocale(lang))return {};
  return {title:getDictionary(lang).orderStatus.title,robots:{index:false,follow:false},referrer:'no-referrer'};
}
export default async function OrderStatusPage({params,searchParams}:{params:Promise<{lang:string;orderNumber:string}>;searchParams:Promise<{token?:string}>}){
  ...
  return <main id="main-content" className="page-shell prose order-status"><p className="eyebrow">{dict.eyebrow}</p><h1>{order.order_number}</h1><p>{dict.orderedOn(formatDate(order.created_at,lang))}</p>{['refunded','cancelled'].includes(order.order_status)?<div className="panel"><h2>{order.order_status==='refunded'?dict.refundedHeading:dict.cancelledHeading}</h2><p>{dict.supportNote}</p></div>:<><h2>{order.payment_status==='paid'?dict.paidHeading:dict.pendingHeading}</h2><ol className="order-progress">{dict.steps.map((step,index)=><li key={step} className={index<=stage?'done':''}><span>{index<=stage?<Check size={18}/>:index+1}</span>{step}</li>)}</ol>{order.order_status==='manual_review'&&<p>{dict.manualReview}</p>}</>}{shipments.map(s=>{const url=safeTrackingUrl(s.tracking_url);return <section className="panel" key={s.id}><Truck size={28}/><h2>{dict.shipmentHeading}</h2><p>{s.carrier} · {s.tracking_number}</p>{s.carrier.includes('Mock')&&<p>{dict.mockShipment}</p>}{url&&<a className="button button-dark" href={url} target="_blank" rel="noopener noreferrer">{dict.trackShipment}</a>}</section>;})}<section className="panel"><Package size={25}/><h2>{dict.orderHeading}</h2>{items.map(i=><p key={i.id}>{i.quantity} × {i.product_name} · {formatCHF(i.total_price_cents)}</p>)}<p>{dict.shipping}: {formatCHF(order.shipping_cents)}</p><strong>{dict.total}: {formatCHF(order.total_cents)}</strong></section><p>{dict.confidentialNote}</p><Link href={`/${lang}/kontakt`}>{dict.questions}</Link></main>;
}
