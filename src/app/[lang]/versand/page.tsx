...
export default async function Page({params}:{params:Promise<{lang:string}>}){
  ...
  return <main id="main-content" className="page-shell prose"><p className="eyebrow">{dict.eyebrow}</p><h1>{dict.heading}</h1><h2>{dict.areaHeading}</h2><p>{dict.areaText}</p><h2>{dict.timeHeading}</h2><p>{s.shipping_text}</p><h2>{dict.costHeading}</h2><p>{s.shipping_cost_cents===null?dict.costUnknown:dict.costKnown(formatCHF(s.shipping_cost_cents))}</p><h2>{dict.originHeading}</h2><p>{s.shipping_origin||dict.originDefault}</p><h2>{dict.trackingHeading}</h2><p>{dict.trackingText}</p><h2>{dict.damagedHeading}</h2><p>{dict.damagedText}</p><Link className="button button-dark" href={`/${lang}/kontakt`}>{dict.contactCta}</Link></main>;
}
