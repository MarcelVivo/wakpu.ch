...
export function checkoutParameters(order:CheckoutSnapshot,origin:string,shippingText:string,expiresAt:number):Stripe.Checkout.SessionCreateParams {
  if(shippingText.length>1200)throw new Error('SHIPPING_TEXT_TOO_LONG');
  return {
    mode:'payment',locale:'de',currency:'chf',adaptive_pricing:{enabled:false},client_reference_id:order.order_id,
    ...
    cancel_url:`${origin}/?checkout=cancelled`,expires_at:expiresAt,
  };
}
