...
export function MobileCartBar({ products }: { products: Product[] }) {
  ...
  if (pathname !== "/" || !product || !variant || !visible || isOpen || settings.shop_maintenance) return null;
  return <div className="mobile-cart-bar"><div><strong>{product.name}</strong><span>{formatPrice(variant.price_chf_cents)}</span></div><AddToCartButton variantId={variant.id} compact label="JETZT BESTELLEN" /></div>;
}
