...
create table public.orders (
  id uuid primary key default gen_random_uuid(),
  order_number text not null unique default ('WK-' || nextval('public.order_number_seq')::text),
...
create index orders_open_idx on public.orders(order_status, fulfillment_status) where payment_status = 'paid';
create index order_items_order_id_idx on public.order_items(order_id);
create index shipments_order_id_idx on public.shipments(order_id);
create index jobs_ready_idx on public.jobs(kind, run_after) where status in ('pending','processing');
create index email_events_order_id_idx on public.email_events(order_id);
create index fulfillment_orders_status_idx on public.fulfillment_orders(status);
...
create policy active_products on public.products for select to anon, authenticated using (active);
create policy active_variants on public.product_variants for select to anon, authenticated using (active and exists (select 1 from public.products p where p.id = product_id and p.active));
create policy active_verified_claims on public.verified_claims for select to anon, authenticated using (active);
grant select on public.products to anon, authenticated;
-- supplier_sku is deliberately not public. Never use select('*') from a client.
grant select (id, product_id, sku, name, price_chf_cents, active, stock_mode, created_at, updated_at) on public.product_variants to anon, authenticated;
grant select (id, title, description, active, display_order, created_at, updated_at) on public.verified_claims to anon, authenticated;
...
create view public.site_public_settings as
...
grant select on public.site_public_settings to anon, authenticated, service_role;
...
begin
  if p_request_id is not null then
    ...
    if found then
      ...
      select coalesce(jsonb_agg(to_jsonb(i) order by i.sku), '[]'::jsonb) into v_snapshot from public.order_items i where i.order_id = v_order.id;
  ...
  insert into public.orders(checkout_request_id,cart_fingerprint,subtotal_cents,shipping_cents,total_cents)
  ...
  insert into public.order_items(order_id,product_id,variant_id,sku,product_name,variant_name,quantity,unit_price_cents,total_price_cents,supplier_sku)
...
create function public.freeze_checkout_params(p_order_id uuid,p_params jsonb)
returns jsonb language plpgsql security definer set search_path = '' as $$
declare v_order public.orders%rowtype; v_subtotal bigint; v_shipping integer;
begin
  ...
  update public.orders set stripe_checkout_params=p_params where id=v_order.id;
  return p_params;
end;
$$;
...
declare v_order public.orders%rowtype; v_inserted uuid; v_refunded integer := 0;
begin
  select * into v_order from public.orders where id = p_order_id for update;
  if not found then raise exception 'ORDER_NOT_FOUND'; end if;
  if exists(select 1 from public.webhook_events where provider='stripe' and event_id=p_event_id) then
  ...
  insert into public.webhook_events(provider,event_id,event_type,order_id)
  ...
  select amount_refunded_cents into v_refunded from public.payments where order_id=v_order.id;
  ...
  insert into public.payments(order_id,stripe_payment_intent_id,stripe_checkout_session_id,amount_cents,currency,status)
    ...
    on conflict(order_id) do update set stripe_checkout_session_id=excluded.stripe_checkout_session_id,
      status=case when public.payments.amount_refunded_cents>=public.payments.amount_cents then 'refunded' else excluded.status end;
  if v_order.payment_status='paid' then
    insert into public.jobs(kind,order_id,dedupe_key) values('order_confirmation',v_order.id,'order_confirmation:'||v_order.id) on conflict(dedupe_key) do nothing;
    if v_order.fulfillment_status='pending' then
      insert into public.jobs(kind,order_id,dedupe_key) values('fulfillment',v_order.id,'fulfillment:'||v_order.id) on conflict(dedupe_key) do nothing;
    end if;
  end if;
...
declare v_order public.orders%rowtype; v_inserted uuid; v_refund integer; v_previous_refund integer := 0;
begin
  ...
  if exists(select 1 from public.webhook_events where provider='stripe' and event_id=p_event_id) then return jsonb_build_object('duplicate',true); end if;
  select * into v_order from public.orders where stripe_payment_intent_id=p_payment_intent_id or (p_order_id is not null and id=p_order_id) for update;
  if not found then raise exception 'ORDER_NOT_FOUND'; end if;
  ...
  update public.orders set stripe_payment_intent_id=p_payment_intent_id where id=v_order.id;
  if p_event_type='payment_intent.payment_failed' then
    update public.orders set payment_status='failed' where id=v_order.id and payment_status='pending';
    insert into public.payments(order_id,stripe_payment_intent_id,amount_cents,status)
      values(v_order.id,p_payment_intent_id,v_order.total_cents,'failed')
      on conflict(order_id) do update set status=case when public.payments.status='pending' then 'failed' else public.payments.status end;
  else
    select amount_refunded_cents into v_previous_refund from public.payments where order_id=v_order.id;
    v_previous_refund:=coalesce(v_previous_refund,0);
    v_refund:=greatest(v_previous_refund,p_amount_refunded);
    insert into public.payments(order_id,stripe_payment_intent_id,amount_cents,amount_refunded_cents,status)
      ...
      on conflict(order_id) do update set amount_refunded_cents=greatest(public.payments.amount_refunded_cents,excluded.amount_refunded_cents),status=case when greatest(public.payments.amount_refunded_cents,excluded.amount_refunded_cents)>=public.payments.amount_cents then 'refunded' else public.payments.status end;
    if v_refund>=v_order.total_cents then
      update public.orders set payment_status='refunded',order_status='refunded',fulfillment_status=case when fulfillment_status in ('shipped','delivered') then fulfillment_status else 'manual_review' end where id=v_order.id;
      ...
      update public.jobs set status='failed',locked_by=null,locked_at=null,locked_until=null,last_error='Order refunded' where order_id=v_order.id and kind='fulfillment' and status='pending';
    end if;
    if v_refund>v_previous_refund then
      insert into public.jobs(kind,order_id,dedupe_key,payload) values('refund_confirmation',v_order.id,'refund_confirmation:'||p_event_id,jsonb_build_object('amount_refunded_cents',v_refund-v_previous_refund,'total_refunded_cents',v_refund)) on conflict(dedupe_key) do nothing;
    end if;
  end if;
...
create function public.claim_jobs(p_worker_id text,p_limit integer default 10,p_lease_seconds integer default 300,p_kinds text[] default null)
...
begin
  ...
  with exhausted as (
    update public.jobs set status='failed',locked_by=null,locked_at=null,locked_until=null,last_error='Worker lease expired after final attempt'
    ...
    update public.fulfillment_orders f set status='manual_review',attempt_count=e.attempt_count,
    ...
    update public.email_events m set status='manual_review',attempt_count=e.attempt_count,
      last_error='Worker lease expired after final attempt',locked_until=null
    from exhausted e where e.kind<>'fulfillment' and m.dedupe_key=e.dedupe_key and m.status<>'sent'
  ), escalated as (
    update public.orders set order_status='manual_review',fulfillment_status='manual_review'
    where id in (select order_id from exhausted where kind='fulfillment') and payment_status='paid' returning id
  ) insert into public.jobs(kind,order_id,dedupe_key,payload)
  ...
  ) update public.jobs j set status='processing',attempt_count=j.attempt_count+1,locked_at=now(),locked_until=now()+make_interval(secs=>p_lease_seconds),locked_by=p_worker_id
    from ready where j.id=ready.id returning j.*;
end;
$$;

create function public.complete_job(p_job_id uuid,p_worker_id text) returns boolean language plpgsql security definer set search_path = '' as $$
begin
  update public.jobs set status='completed',locked_at=null,locked_until=null,locked_by=null,last_error=null
    where id=p_job_id and status='processing' and locked_by=p_worker_id and locked_until>now();
  return found;
end;
$$;

create function public.defer_job(p_job_id uuid,p_worker_id text,p_seconds integer) returns boolean language plpgsql security definer set search_path = '' as $$
begin
  if p_seconds not between 5 and 86400 then raise exception 'INVALID_DELAY'; end if;
  update public.jobs set status='pending',attempt_count=greatest(0,attempt_count-1),run_after=now()+make_interval(secs=>p_seconds),locked_at=null,locked_until=null,locked_by=null
    where id=p_job_id and status='processing' and locked_by=p_worker_id and locked_until>now();
  return found;
end;
$$;

create function public.fail_job(p_job_id uuid,p_worker_id text,p_error text) returns boolean language plpgsql security definer set search_path = '' as $$
declare v_job public.jobs%rowtype;
begin
  update public.jobs set status=case when attempt_count>=max_attempts then 'failed' else 'pending' end,
  ...
  if v_job.kind='fulfillment' then
    update public.fulfillment_orders set attempt_count=v_job.attempt_count,
    ...
    update public.email_events set status=case when v_job.status='failed' then 'manual_review' else 'failed' end,
  ...
  if v_job.kind='fulfillment' and v_job.status='failed' then
    update public.orders set order_status='manual_review',fulfillment_status='manual_review' where id=v_job.order_id and payment_status='paid';
    insert into public.jobs(kind,order_id,dedupe_key,payload) values('fulfillment_error',v_job.order_id,'fulfillment_error:'||v_job.order_id,jsonb_build_object('reason',left(p_error,1000))) on conflict(dedupe_key) do nothing;
  end if;
  return true;
end;
$$;

create function public.park_job(p_job_id uuid,p_worker_id text,p_error text) returns boolean language plpgsql security definer set search_path = '' as $$
declare v_job public.jobs%rowtype;
begin
  update public.jobs set status='failed',last_error=left(p_error,1000),locked_by=null,locked_at=null,locked_until=null
    where id=p_job_id and status='processing' and locked_by=p_worker_id and locked_until>now() returning * into v_job;
  if not found then return false; end if;
  if v_job.kind='fulfillment' then
    update public.orders set order_status='manual_review',fulfillment_status='manual_review' where id=v_job.order_id and payment_status='paid';
    update public.fulfillment_orders set status='manual_review',attempt_count=v_job.attempt_count,last_error=left(p_error,1000),locked_until=null where order_id=v_job.order_id and provider_order_id is null;
    insert into public.jobs(kind,order_id,dedupe_key,payload) values('fulfillment_error',v_job.order_id,'fulfillment_error:'||v_job.order_id,jsonb_build_object('reason',left(p_error,1000))) on conflict(dedupe_key) do nothing;
  else
    update public.email_events set status='manual_review',attempt_count=v_job.attempt_count,last_error=left(p_error,1000),locked_until=null
      where dedupe_key=v_job.dedupe_key and status<>'sent';
  end if;
  return true;
end;
...
create function public.begin_fulfillment(
...
declare v_job public.jobs%rowtype; v_order public.orders%rowtype; v_fo public.fulfillment_orders%rowtype; v_settings public.site_settings%rowtype;
begin
  select * into v_job from public.jobs where id=p_job_id and kind='fulfillment' and status='processing'
  ...
  select * into v_fo from public.fulfillment_orders where id=p_fulfillment_id and order_id=v_job.order_id for update;
  ...
  select * into v_settings from public.site_settings where id=true for share;
  ...
  update public.fulfillment_orders set status='submitting',submission_started_at=coalesce(submission_started_at,now()),
...
create function public.retry_fulfillment(p_order_id uuid,p_provider text,p_idempotent_create boolean)
returns boolean language plpgsql security definer set search_path = '' as $$
declare v_job public.jobs%rowtype; v_order public.orders%rowtype; v_fo public.fulfillment_orders%rowtype; v_enabled boolean;
begin
  perform pg_advisory_xact_lock(hashtextextended('retry:'||p_order_id::text,0));
  select * into v_job from public.jobs where dedupe_key='fulfillment:'||p_order_id for update;
  ...
  select fulfillment_enabled into v_enabled from public.site_settings where id=true for share;
  if not coalesce(v_enabled,false) then raise exception 'FULFILLMENT_DISABLED'; end if;
  select * into v_fo from public.fulfillment_orders where order_id=p_order_id for update;
  if found then
    ...
    update public.fulfillment_orders set status='pending',last_error=null,locked_until=null where id=v_fo.id;
  end if;
  update public.orders set order_status='fulfillment_pending',fulfillment_status='pending' where id=p_order_id;
  insert into public.jobs(kind,order_id,dedupe_key) values('fulfillment',p_order_id,'fulfillment:'||p_order_id)
...
create function public.begin_email(p_job_id uuid,p_worker_id text,p_event_id uuid)
returns boolean language plpgsql security definer set search_path = '' as $$
declare v_job public.jobs%rowtype; v_event public.email_events%rowtype;
begin
  select * into v_job from public.jobs where id=p_job_id and kind<>'fulfillment' and status='processing'
    and locked_by=p_worker_id and locked_until>now()+interval '30 seconds' for update;
  if not found then return false; end if;
  select * into v_event from public.email_events where id=p_event_id and order_id=v_job.order_id and dedupe_key=v_job.dedupe_key for update;
  ...
  update public.email_events set status='sending',first_attempt_at=coalesce(first_attempt_at,now()),
    attempt_count=v_job.attempt_count,locked_until=v_job.locked_until,last_error=null where id=v_event.id;
  return true;
end;
$$;

create function public.finish_email(p_job_id uuid,p_worker_id text,p_event_id uuid,p_resend_email_id text)
returns boolean language plpgsql security definer set search_path = '' as $$
declare v_job public.jobs%rowtype; v_event public.email_events%rowtype;
begin
  select * into v_job from public.jobs where id=p_job_id and kind<>'fulfillment' and status='processing'
  ...
  update public.email_events set status='sent',resend_email_id=p_resend_email_id,sent_at=coalesce(sent_at,now()),locked_until=null,last_error=null where id=v_event.id;
  update public.jobs set status='completed',locked_at=null,locked_until=null,locked_by=null,last_error=null where id=v_job.id;
  return true;
end;
$$;

create function public.commit_shipment(
...
declare v_fo public.fulfillment_orders%rowtype; v_order public.orders%rowtype; v_shipment public.shipments%rowtype;
...
begin
  ...
  select * into v_fo from public.fulfillment_orders where id=p_fulfillment_id;
  if not found then raise exception 'FULFILLMENT_ORDER_NOT_FOUND'; end if;
  select * into v_order from public.orders where id=v_fo.order_id for update;
  ...
  select * into v_shipment from public.shipments where fulfillment_order_id=v_fo.id and tracking_number=p_tracking_number for update;
  ...
  insert into public.shipments(order_id,fulfillment_order_id,carrier,tracking_number,tracking_url,status,shipped_at,delivered_at)
    ...
    on conflict(fulfillment_order_id,tracking_number) do update set carrier=excluded.carrier,tracking_url=excluded.tracking_url,status=excluded.status,
      shipped_at=coalesce(public.shipments.shipped_at,excluded.shipped_at),delivered_at=coalesce(excluded.delivered_at,public.shipments.delivered_at);
  ...
  update public.fulfillment_orders set tracking_number=p_tracking_number,tracking_url=p_tracking_url,
    status=case when v_safe then v_order_state else 'manual_review' end where id=v_fo.id;
  if v_safe then
    update public.orders set fulfillment_status=v_order_state,order_status=v_order_state where id=v_order.id;
    insert into public.jobs(kind,order_id,dedupe_key,payload) values(v_kind,v_order.id,'tracking:'||v_fo.id||':'||p_tracking_number||':'||p_status,
    ...
    if p_status='exception' then
      insert into public.jobs(kind,order_id,dedupe_key,payload) values('fulfillment_error',v_order.id,'fulfillment_error:'||v_order.id,jsonb_build_object('reason','Supplier reported a shipment exception')) on conflict(dedupe_key) do nothing;
    end if;
  end if;
  return true;
end;
...
create function public.finish_fulfillment(p_job_id uuid,p_worker_id text,p_fulfillment_id uuid,p_provider_order_id text,p_status text,p_cost_cents integer)
returns boolean language plpgsql security definer set search_path = '' as $$
declare v_job public.jobs%rowtype; v_order public.orders%rowtype; v_fo public.fulfillment_orders%rowtype;
  v_limit integer; v_cost_safe boolean; v_order_safe boolean;
begin
  select * into v_job from public.jobs where id=p_job_id and kind='fulfillment' and status='processing' and locked_by=p_worker_id and locked_until>now() for update;
  if not found then return false; end if;
  ...
  select max_supplier_order_cost_cents into v_limit from public.site_settings where id=true for share;
  ...
  update public.fulfillment_orders set provider_order_id=p_provider_order_id,
  ...
  if v_order_safe and v_cost_safe then
    update public.orders set fulfillment_status=p_status,order_status=case when p_status='submitted' then 'fulfillment_submitted' else p_status end where id=v_job.order_id;
  else
    update public.orders set fulfillment_status='manual_review',order_status=case when order_status in ('refunded','cancelled') then order_status else 'manual_review' end where id=v_job.order_id;
    insert into public.jobs(kind,order_id,dedupe_key,payload) values('fulfillment_error',v_job.order_id,'fulfillment_error:'||v_job.order_id,
  ...
  update public.jobs set status='completed',locked_at=null,locked_until=null,locked_by=null,last_error=null where id=p_job_id;
  return true;
end;
$$;

create function public.dashboard_metrics() returns jsonb language sql security definer set search_path = '' as $$
...
begin
  ...
  update public.products set name=p_name,description=p_description,short_description=p_short_description,active=p_active,featured=p_featured where id=p_product_id;
  update public.product_variants set sku=p_sku,supplier_sku=nullif(trim(p_supplier_sku),''),price_chf_cents=p_price_chf_cents,stock_mode=p_stock_mode,active=p_active where id=p_variant_id;
  return true;
end;
$$;
...
revoke all on function public.create_pending_order(jsonb,uuid,text) from public,anon,authenticated;
revoke all on function public.freeze_checkout_params(uuid,jsonb) from public,anon,authenticated;
revoke all on function public.finalize_paid_order(text,uuid,text,text,integer,text,jsonb) from public,anon,authenticated;
revoke all on function public.record_payment_event(text,text,text,integer,uuid) from public,anon,authenticated;
revoke all on function public.claim_jobs(text,integer,integer,text[]) from public,anon,authenticated;
revoke all on function public.complete_job(uuid,text) from public,anon,authenticated;
revoke all on function public.defer_job(uuid,text,integer) from public,anon,authenticated;
revoke all on function public.fail_job(uuid,text,text) from public,anon,authenticated;
revoke all on function public.park_job(uuid,text,text) from public,anon,authenticated;
revoke all on function public.finish_fulfillment(uuid,text,uuid,text,text,integer) from public,anon,authenticated;
revoke all on function public.dashboard_metrics() from public,anon,authenticated;
revoke all on function public.admin_update_product(uuid,uuid,text,text,text,boolean,boolean,text,text,integer,text) from public,anon,authenticated;
revoke all on function public.begin_fulfillment(uuid,text,uuid,integer,boolean,timestamptz) from public,anon,authenticated;
revoke all on function public.retry_fulfillment(uuid,text,boolean) from public,anon,authenticated;
revoke all on function public.commit_shipment(uuid,text,text,text,text,timestamptz,timestamptz) from public,anon,authenticated;
revoke all on function public.begin_email(uuid,text,uuid) from public,anon,authenticated;
revoke all on function public.finish_email(uuid,text,uuid,text) from public,anon,authenticated;
grant execute on function public.create_pending_order(jsonb,uuid,text) to service_role;
grant execute on function public.freeze_checkout_params(uuid,jsonb) to service_role;
grant execute on function public.finalize_paid_order(text,uuid,text,text,integer,text,jsonb) to service_role;
grant execute on function public.record_payment_event(text,text,text,integer,uuid) to service_role;
grant execute on function public.claim_jobs(text,integer,integer,text[]) to service_role;
grant execute on function public.complete_job(uuid,text) to service_role;
grant execute on function public.defer_job(uuid,text,integer) to service_role;
grant execute on function public.fail_job(uuid,text,text) to service_role;
grant execute on function public.park_job(uuid,text,text) to service_role;
grant execute on function public.finish_fulfillment(uuid,text,uuid,text,text,integer) to service_role;
grant execute on function public.dashboard_metrics() to service_role;
grant execute on function public.admin_update_product(uuid,uuid,text,text,text,boolean,boolean,text,text,integer,text) to service_role;
grant execute on function public.begin_fulfillment(uuid,text,uuid,integer,boolean,timestamptz) to service_role;
grant execute on function public.retry_fulfillment(uuid,text,boolean) to service_role;
grant execute on function public.commit_shipment(uuid,text,text,text,text,timestamptz,timestamptz) to service_role;
grant execute on function public.begin_email(uuid,text,uuid) to service_role;
grant execute on function public.finish_email(uuid,text,uuid,text) to service_role;
