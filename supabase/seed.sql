-- Re-runnable seed for the wakpu schema only. Existing prices, business details
-- and settings are preserved.
-- Only the database owns prices; application source contains no seeded amounts.
begin;
insert into wakpu.products(id,name,slug,description,short_description,active,featured,badge,display_order)
values
...
insert into wakpu.product_variants(id,product_id,sku,name,price_chf_cents,supplier_sku,active,stock_mode)
select 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaa1',id,'WAKPU-MYSTERY-1','Mystery Color',990,'MOCK-WAKPU-1',true,'available' from wakpu.products where slug='wakpu-single'
on conflict(sku) do nothing;
insert into wakpu.product_variants(id,product_id,sku,name,price_chf_cents,supplier_sku,active,stock_mode)
select 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaa3',id,'WAKPU-MYSTERY-3','Mystery Color',2490,'MOCK-WAKPU-3',true,'available' from wakpu.products where slug='wakpu-triple'
on conflict(sku) do nothing;
insert into wakpu.product_variants(id,product_id,sku,name,price_chf_cents,supplier_sku,active,stock_mode)
select 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaa6',id,'WAKPU-MYSTERY-6','Mystery Color',3990,'MOCK-WAKPU-6',true,'available' from wakpu.products where slug='wakpu-party-pack'
on conflict(sku) do nothing;
...
-- New installations keep maintenance enabled and live supplier fulfillment disabled.
commit;
