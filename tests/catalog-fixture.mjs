...
await db.exec("update wakpu.site_settings set shop_maintenance=false,default_shipping_text='Testbetrieb: Lieferzeit wird vor dem Verkaufsstart bestätigt.';");
const { rows: catalog } = await db.query(`select p.*, coalesce((select jsonb_agg(v order by v.created_at) from wakpu.product_variants v where v.product_id=p.id and v.active), '[]'::jsonb) as variants from wakpu.products p where p.active order by p.display_order`);
const { rows: settings } = await db.query('select * from wakpu.site_settings');
...
const server = createServer((request, response) => {
  ...
  // Fail closed if an application client accidentally selects the shared public
  // schema. The standalone browser-test metadata routes are not PostgREST.
  if (pathname.startsWith('/rest/v1/') && request.headers['accept-profile'] !== 'wakpu') {
    response.writeHead(406); response.end(JSON.stringify({ code: 'PGRST106', message: 'The application must select the wakpu schema' })); return;
  }
